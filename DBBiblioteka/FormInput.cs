...
namespace DBBiblioteka
{
    public partial class FormInput : MetroFramework.Forms.MetroForm
    {
        ...
        private void PopulateControls()
        {
            ...
            foreach (PropertyInfo item in myInterface.GetType().GetProperties())
            {
                if (item.GetCustomAttribute<ForeignKeyAttribute>() != null)
                {
                    ...
                    {
                        //ul.PromjenaTipaClanarine += PromijeniDatumIstekaClanarine;
                        ul.PromjenaTipaClanarine += (s, tipClanarine) =>
                        {
                            if (tipClanarine == 1)
                                datoom.SetValue(DateTime.Now.AddMonths(1));
                            else if (tipClanarine == 2)
                                datoom.SetValue(DateTime.Now.AddMonths(6));
                            else if (tipClanarine == 3)
                                datoom.SetValue(DateTime.Now.AddYears(1));
                            else
                                datoom.SetValue(DateTime.Now);
                        };
                        //ul.AddTextChangedEvent();////////////////////////// dodaje jos jedan event za automatsko mijenjanje datuma za tip clanarine
                        //SVA TRI NACINA RADE ISTO
                    }
                ...
                else if (item.GetCustomAttribute<DateTimeAttribute>() != null)
                {
                    if (state == StateEnum.Create || state == StateEnum.Update)
                    {
                        ...
                        if (dtc.Name == "DatumIstekaClanarine")
                        {
                            datoom = dtc;
                        }
                    }
        ...
        public static void PromijeniDatumIstekaClanarine(object sender, int tipClanarine)
        {
